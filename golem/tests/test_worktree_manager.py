...
class TestFastForwardIfSafe:
    ...
    def test_stash_if_dirty_succeeds(self, git_repo, tmp_path):
        """With stash_if_dirty=True, dirty overlapping files are stashed, ff proceeds, stash popped."""
        from golem.worktree_manager import fast_forward_if_safe

        # Set up a multi-line README so merge and user edit different parts
        (git_repo / "README.md").write_text("line1\nline2\nline3\n")
        _run_git(["add", "."], cwd=str(git_repo))
        _run_git(["commit", "-m", "multi-line readme"], cwd=str(git_repo))

        _run_git(["checkout", "-b", "merge-ready/200"], cwd=str(git_repo))
        (git_repo / "README.md").write_text("MERGED\nline2\nline3\n")
        _run_git(["add", "."], cwd=str(git_repo))
        _run_git(["commit", "-m", "edit line 1"], cwd=str(git_repo))
        _run_git(["checkout", "master"], cwd=str(git_repo))

        # Dirty a different part of the same file
        (git_repo / "README.md").write_text("line1\nline2\nUSER EDIT\n")

        ok, reason = fast_forward_if_safe(
            str(git_repo), "merge-ready/200", stash_if_dirty=True
        )
        assert ok is True
        assert reason == ""
        # User's dirty changes restored via stash pop — both edits present
        content = (git_repo / "README.md").read_text()
        assert "MERGED" in content
        assert "USER EDIT" in content

    def test_stash_if_dirty_false_still_defers(self, git_repo, tmp_path):
        """Default stash_if_dirty=False preserves existing defer behavior."""
        from golem.worktree_manager import fast_forward_if_safe

        _run_git(["checkout", "-b", "merge-ready/201"], cwd=str(git_repo))
        (git_repo / "README.md").write_text("merged version\n")
        _run_git(["add", "."], cwd=str(git_repo))
        _run_git(["commit", "-m", "edit readme"], cwd=str(git_repo))
        _run_git(["checkout", "master"], cwd=str(git_repo))
        ...
        ok, reason = fast_forward_if_safe(str(git_repo), "merge-ready/201")
        assert ok is False
        assert "dirty" in reason

    def test_stash_if_dirty_preserves_unrelated_changes(self, git_repo, tmp_path):
        """Stash+pop preserves dirty files that don't overlap with merge."""
        from golem.worktree_manager import fast_forward_if_safe

        _run_git(["checkout", "-b", "merge-ready/202"], cwd=str(git_repo))
        (git_repo / "README.md").write_text("merged version\n")
        _run_git(["add", "."], cwd=str(git_repo))
        _run_git(["commit", "-m", "edit readme"], cwd=str(git_repo))
        _run_git(["checkout", "master"], cwd=str(git_repo))

        # Dirty an overlapping file AND an unrelated file
        (git_repo / "README.md").write_text("user edits\n")
        (git_repo / "other.txt").write_text("unrelated wip\n")

        ok, reason = fast_forward_if_safe(
            str(git_repo), "merge-ready/202", stash_if_dirty=True
        )
        assert ok is True
        assert reason == ""
        # Unrelated dirty file preserved
        assert (git_repo / "other.txt").read_text() == "unrelated wip\n"

    def test_stash_if_dirty_pop_conflict_still_succeeds(self, git_repo, tmp_path):
        """If stash pop conflicts, the ff merge still succeeds and stash is preserved."""
        from golem.worktree_manager import fast_forward_if_safe

        # Create branch that modifies README to conflicting content
        _run_git(["checkout", "-b", "merge-ready/203"], cwd=str(git_repo))
        (git_repo / "README.md").write_text("line1\nmerge content\nline3\n")
        _run_git(["add", "."], cwd=str(git_repo))
        _run_git(["commit", "-m", "edit readme"], cwd=str(git_repo))
        _run_git(["checkout", "master"], cwd=str(git_repo))

        # Dirty the same file with conflicting content
        (git_repo / "README.md").write_text("line1\nuser content\nline3\n")

        ok, reason = fast_forward_if_safe(
            str(git_repo), "merge-ready/203", stash_if_dirty=True
        )
        # Merge succeeds even if pop conflicts
        assert ok is True
        assert reason == ""

    def test_stash_if_dirty_stash_fails(self, monkeypatch, tmp_path):
        """When git stash itself fails, fall back to deferred."""
        from golem.worktree_manager import fast_forward_if_safe
        ...
        def mock_run_git(args, cwd, timeout=30):
            ...
            if args[0] == "stash":
                result.returncode = 1
                result.stdout = ""
                result.stderr = "Cannot save the current worktree state"
                return result
            result.returncode = 0
            result.stdout = ""
            result.stderr = ""
        ...
        monkeypatch.setattr("golem.worktree_manager._run_git", mock_run_git)
        ok, reason = fast_forward_if_safe(
            str(tmp_path / "repo"), "branch", stash_if_dirty=True
        )
        assert ok is False
        assert "dirty" in reason

    def test_stash_if_dirty_merge_still_fails(self, monkeypatch, tmp_path):
        """When stash succeeds but retry merge still fails, return failure."""
        from golem.worktree_manager import fast_forward_if_safe
