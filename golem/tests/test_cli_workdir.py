...
from golem.core.cli_wrapper import _copy_subdir, _prepare_work_dir
...
class TestCopySubdir:
    ...
    def test_copies_directory_not_symlink(self, tmp_path):
        ...
        created: list[Path] = []
        with patch("golem.core.cli_wrapper._PROJECT_CLAUDE_DIR", src_claude):
...
class TestPrepareWorkDir:
    ...
    def test_skills_are_copied_not_symlinked(self, tmp_path):
        ...
        with (
            patch("golem.core.cli_wrapper._PROJECT_ROOT", project_root),
            patch("golem.core.cli_wrapper._PROJECT_CLAUDE_DIR", src_claude),
        ):
