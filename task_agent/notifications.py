"""Teams card builders for task-agent events.

Constructs Microsoft Teams Adaptive Cards for the key lifecycle moments of a
task-agent session: session start, successful completion, mid-run activity
updates, failure (agent crash), and escalation (validation did not pass).

Key exports:
- ``build_task_started_card`` — card sent when a task-agent session begins.
...
def build_task_started_card(
    ...
    """Card sent when a task-agent session starts execution."""
    body: list[dict[str, Any]] = [
        _header_block(f"Task Agent Started: #{parent_id}", color="accent"),
...
def build_task_completed_card(
    ...
    *,
    ...
    body: list[dict[str, Any]] = [
        _header_block(f"Task Agent Completed: #{parent_id}", color=color),
        _text_block(subject[:120]),
    ]
...
def build_task_activity_card(
    ...
    body: list[dict[str, Any]] = [
        _header_block(f"Task Agent: #{parent_id} — In Progress", color="accent"),
...
def build_task_failure_card(
    ...
    *,
    ...
    body: list[dict[str, Any]] = [
        _header_block(f"Task Agent Failed: #{parent_id}", color="attention"),
        _text_block(subject[:120]),
        _fact_set(facts),
    ]
    actions = [_open_url_action("View Issue", f"{REDMINE_ISSUES_URL}/{parent_id}")]
...
def build_task_escalation_card(
    ...
    body: list[dict[str, Any]] = [
        _header_block(f"Task Agent Needs Review: #{parent_id}", color="warning"),
        _text_block(subject[:120]),
        _fact_set(
            [
                ("Verdict", verdict),
