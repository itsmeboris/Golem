"""Durable state-machine orchestrator for task-agent sessions (v2).
...
After execution the orchestrator runs a 5-phase pipeline:
  1. Execute — invoke the task agent with event-stream monitoring
...
from .core.config import DATA_DIR, PROJECT_ROOT, TaskAgentFlowConfig
...
from .profile import TaskAgentProfile
...
_REPORT_DIR = DATA_DIR / "reports" / "task_agent"
_REPORT_INDEX = DATA_DIR / "reports" / "task_agent_report.md"
...
class TaskSessionState(str, Enum):
    """Lifecycle states for a task-agent session (v2).
...
class TaskOrchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        profile: TaskAgentProfile | None = None,
    ...
    def _persist_traces(
        ...
        event_id = f"task_agent-{issue_id}"
        if prompt:
            _write_prompt("task_agent", event_id, prompt)
        if result and result.trace_events:
            self.session.trace_file = _write_trace(
                "task_agent", event_id, result.trace_events
            )
    ...
    def _handle_agent_failure(
        ...
        self._update_task(
            issue_id,
            comment=f"Task agent failed after {format_duration(elapsed)}: {exc}",
        )
    ...
    async def _retry_agent(  # pylint: disable=too-many-locals
        ...
        # Persist retry trace
        retry_event_id = f"task_agent-{issue_id}-retry"
        _write_prompt("task_agent", retry_event_id, retry_prompt)
        if retry_result and retry_result.trace_events:
            self.session.retry_trace_file = _write_trace(
                "task_agent", retry_event_id, retry_result.trace_events
            )
    ...
    def _escalate(self, verdict: ValidationVerdict) -> None:
        ...
        notes = (
            f"**Task agent escalation — needs human review**\n\n"
