...
def _get_profile(config):
    """Build a TaskAgentProfile from config.  Raises on failure."""
    tc = config.get_flow_config("task_agent")
...
def _manage_task_agent_tick(config, tasks):
    """Start the task-agent tick loop via TaskAgentFlow."""
    ...
    if hasattr(flow, "start_tick_loop"):
        ...
        print("Task-agent tick loop started")
    return flow
...
async def _start_dashboard_server(
    ...
    app = FastAPI(title="Task Agent Dashboard")
    mount_dashboard(
...
async def run_daemon(args, config) -> int:
    """Start the task-agent daemon with tick loop and dashboard."""
    ...
    flow = _manage_task_agent_tick(config, tasks)
    if flow is None:
        print("Task agent is not enabled in config", file=sys.stderr)
        return 1
...
def cmd_status(args) -> int:
    ...
    print(format_status_text(since_hours=since, flow="task_agent"))
    return 0
...
def main() -> int:
    ...
    # daemon
    daemon_p = sub.add_parser("daemon", help="Run task agent daemon with tick loop")
