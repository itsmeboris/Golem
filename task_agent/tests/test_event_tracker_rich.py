"""Tests for rich tool-call summaries in the task-agent event tracker."""

# pylint: disable=missing-class-docstring,missing-function-docstring

from task_agent.event_tracker import (
    TaskEventTracker,
    _short_path,
...
class TestRichSummaryIntegration:
    ...
    def test_read_event(self):
        tracker = TaskEventTracker(session_id=1)
        m = tracker.handle_event(_tool_use_event("Read", {
    ...
    def test_mcp_event(self):
        tracker = TaskEventTracker(session_id=1)
        m = tracker.handle_event(_tool_use_event("mcp__redmine__redmine_get_issue", {
            "issue_id": 123,
        }))
