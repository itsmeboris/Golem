"""Integration tests for the task-agent profile system.
...
from task_agent.core.config import Config, TaskAgentFlowConfig
...
def _make_test_profile(tasks_dir="./tasks", name="test"):
    """Build a TaskAgentProfile with null backends for testing."""
    from task_agent.backends.local import (
        LocalFileTaskSource,
        LogNotifier,
        NullStateBackend,
        NullToolProvider,
    )
    from task_agent.profile import TaskAgentProfile
...
class TestProfileRegistry:
    def test_build_redmine_profile(self):
        from task_agent.profile import build_profile

        config = Config(task_agent=TaskAgentFlowConfig())
    ...
    def test_unknown_profile_raises(self):
        ...
        config = Config(task_agent=TaskAgentFlowConfig())
        with pytest.raises(ValueError, match="Unknown task-agent profile"):
            build_profile("nonexistent_xyz", config)

    def test_available_profiles_lists_builtins(self):
        from task_agent.profile import available_profiles, build_profile
    ...
    def test_custom_profile_registration(self):
        from task_agent.profile import (
            TaskAgentProfile,
            _PROFILE_FACTORIES,
            register_profile,
            build_profile,
...
class TestProtocolCompliance:
    def test_redmine_backends_satisfy_protocols(self):
        from task_agent.interfaces import (
            Notifier,
            PromptProvider,
            StateBackend,
            TaskSource,
...
class TestLocalFileTaskSource:
    def test_poll_tasks_finds_agent_tagged(self, tmp_path):
        from task_agent.backends.local import LocalFileTaskSource
...
class TestNullStateBackend:
    def test_update_status_returns_true(self):
        from task_agent.backends.local import NullStateBackend
...
class TestLogNotifier:
    def test_notify_started_does_not_raise(self):
        from task_agent.backends.local import LogNotifier
...
class TestNullToolProvider:
    def test_base_servers_empty(self):
        from task_agent.backends.local import NullToolProvider
...
class TestProfileSwitching:
    def test_redmine_profile_has_redmine_backends(self):
        from task_agent.backends.redmine import (
            RedmineStateBackend,
            RedmineTaskSource,
        )
        from task_agent.profile import build_profile

        config = Config(task_agent=TaskAgentFlowConfig(profile="redmine"))
        profile = build_profile("redmine", config)
...
class TestFlowWithProfile:
    def _make_flow(self, monkeypatch, tmp_path, profile=None):
        """Build a TaskAgentFlow, injecting *profile* via monkeypatch.
    ...
    def test_flow_passes_profile_to_orchestrator(self, monkeypatch, tmp_path):
        ...
        from task_agent.orchestrator import TaskOrchestrator
...
class TestNullProfileEndToEnd:
    def test_poll_detect_handle_lifecycle(  # pylint: disable=too-many-locals
        ...
        from task_agent.flow import TaskAgentFlow
        from task_agent.orchestrator import TaskSessionState
        from task_agent.core.triggers.base import TriggerEvent
...
class TestRedmineBackendMocked:
    def test_redmine_state_backend_update_status(self, monkeypatch):
        from task_agent.backends.redmine import RedmineStateBackend
        ...
        monkeypatch.setattr(
            "task_agent.backends.redmine._requests.put", mock_put
        )
        monkeypatch.setattr(
            "task_agent.backends.redmine._requests.get", mock_get
        )
    ...
    def test_redmine_task_source_get_description(self, monkeypatch):
        from task_agent.backends.redmine import RedmineTaskSource
