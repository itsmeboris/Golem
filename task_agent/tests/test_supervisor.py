"""Tests for the task-agent supervisor/worker architecture."""
...
class TestSubtaskResult:
    def test_defaults(self):
        from task_agent.orchestrator import SubtaskResult
...
class TestTaskSessionSupervisorFields:
    def test_new_fields_defaults(self):
        from task_agent.orchestrator import TaskSession
...
class TestTaskSupervisor:
    def test_build_sibling_status_empty(self):
        from task_agent.supervisor import TaskSupervisor
        from task_agent.orchestrator import TaskSession

        session = TaskSession(parent_issue_id=100)
        sup = TaskSupervisor(session, MagicMock(), TaskAgentFlowConfig())
        result = sup._build_sibling_status([])
        assert "No prior subtasks" in result

    def test_build_sibling_status_with_results(self):
        from task_agent.supervisor import TaskSupervisor
        from task_agent.orchestrator import SubtaskResult, TaskSession
...
class TestOrchestratorBranching:
    def test_supervisor_mode_dispatches(self):
        ...
        config = MagicMock()
        task_config = TaskAgentFlowConfig(supervisor_mode=True)
        orch = TaskOrchestrator(session, config, task_config)

        import asyncio

        with patch("task_agent.supervisor.TaskSupervisor") as mock_sup_cls:
    ...
    def test_monolithic_mode_skips_supervisor(self):
        ...
        config = MagicMock()
        task_config = TaskAgentFlowConfig(supervisor_mode=False)
        orch = TaskOrchestrator(session, config, task_config)
...
def _mock_profile():
    """Create a mock TaskAgentProfile with null-like backends."""
    profile = MagicMock()
    profile.name = "test"
...
class TestCommitAndCompleteMerge:
    ...
    def test_merge_called_when_no_supervisor_commit(self):
        ...
        from task_agent.supervisor import TaskSupervisor
        from task_agent.orchestrator import TaskSession
        from task_agent.committer import CommitResult
        from task_agent.validation import ValidationVerdict

        session = TaskSession(parent_issue_id=500, parent_subject="[AGENT] Test")
        task_config = TaskAgentFlowConfig(auto_commit=True)
        ...
        with patch(
            "task_agent.supervisor.commit_changes",
            return_value=CommitResult(committed=False, message="No changes to commit"),
        ), patch(
            "task_agent.supervisor.merge_and_cleanup",
            return_value="abc1234",
        ) as mock_merge:
            sup._commit_and_complete(500, "/repo/data/agent/worktrees/500", verdict)

        mock_merge.assert_called_once_with("/repo", 500, "/repo/data/agent/worktrees/500")
...
class TestThrottledCheckpoint:
    ...
    def _make_orchestrator(self, save_callback=None):
        from task_agent.orchestrator import TaskOrchestrator, TaskSession
    ...
    def _make_milestone(self, ts=0.0):
        ...
        return Milestone(kind="tool_call", tool_name="Read", summary="Read file", timestamp=ts)
