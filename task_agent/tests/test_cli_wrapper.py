"""Tests for task-agent core.cli_wrapper — settings.json, MCP filtering, event handler."""
...
from task_agent.core.cli_wrapper import (
    CLIConfig,
    CLIType,
    _copy_mcp_json,
    _get_subprocess_env,
...
from task_agent.core.stream_printer import StreamPrinter
...
class TestWriteSettingsJson:
    ...
    def test_extracts_mcp_hooks_from_project(self, tmp_path):
        ...
        created: list[Path] = []
        with patch(
            "task_agent.core.cli_wrapper._PROJECT_CLAUDE_DIR",
...
class TestCopyMcpJson:
    ...
    def test_empty_list_skips_copy(self, tmp_path):
        ...
        source.write_text(
            json.dumps({"mcpServers": {"redmine": {}, "jenkins": {}}})
        )

        created: list[Path] = []
        with patch(
            "task_agent.core.cli_wrapper._PROJECT_MCP_JSON", source
        ):
    ...
    def test_skips_if_source_missing(self, tmp_path):
        ...
        with patch(
            "task_agent.core.cli_wrapper._PROJECT_MCP_JSON",
...
class TestMakeEventHandler:
    ...
    def _make_handler(self, session=None, start_time=None):
        ...
        from task_agent.cli import _make_event_handler

        tracker = TaskEventTracker(session_id=999)
        printer = MagicMock(spec=StreamPrinter)
        return _make_event_handler(tracker, printer, session=session, start_time=start_time)
    ...
    def test_session_gets_milestone_count(self):
        ...
        with patch("task_agent.cli._save_cli_session"):
            handler(event)
    ...
    def test_save_failure_does_not_break_execution(self):
        ...
        with patch(
            "task_agent.cli._save_cli_session",
...
class TestProjectRootSandbox:
    ...
    def test_cwd_at_project_root_gets_temp_sandbox(self, tmp_path):
        ...
        with patch(
            "task_agent.core.cli_wrapper._PROJECT_ROOT", tmp_path
        ):
            config = CLIConfig(
    ...
    def test_explicit_worktree_still_used_directly(self, tmp_path):
        ...
        with patch(
            "task_agent.core.cli_wrapper._PROJECT_ROOT",
            tmp_path / "project",
        ):
            config = CLIConfig(
