...
Each task-agent session gets its own git worktree so multiple agents can work
...
def merge_and_cleanup(
    ...
    rebase_result = _run_git(
        ["rebase", target_branch, branch_name], cwd=base_dir
    )
    if rebase_result.returncode != 0:
        logger.warning(
            "Session #%s: rebase onto %s failed — %s",
            issue_id, target_branch, rebase_result.stderr.strip(),
        )
    ...
    merge_result = _run_git(
        ["merge", "--ff-only", branch_name], cwd=base_dir,
    )
    ...
    if merge_result.returncode == 0:
        sha_result = _run_git(
            ["rev-parse", "--short", "HEAD"], cwd=base_dir
        )
        sha = sha_result.stdout.strip()
        logger.info(
            "Session #%s: merged branch %s → %s",
            issue_id, branch_name, sha,
        )
    else:
        logger.error(
