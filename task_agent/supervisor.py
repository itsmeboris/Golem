"""Supervisor/worker execution engine for task-agent sessions.
...
from .core.config import PROJECT_ROOT, TaskAgentFlowConfig
...
class TaskSupervisor:
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            self._update_task(
                issue_id,
                comment=f"Task agent (supervisor) failed: {exc}",
            )
    ...
    async def _decompose(self, issue_id: int, work_dir: str) -> list[dict[str, Any]]:
        ...
        self._persist_trace(f"task_agent-{issue_id}-decompose", prompt, result)
    ...
    async def _execute_subtask(
        ...
        try:
            ...
            self._update_task(child_id, comment=f"Subtask agent failed: {exc}")
            return SubtaskResult(
                issue_id=child_id,
                subject=child_subject,
                status="failed",
    ...
    async def _invoke_subtask(
        ...
        """Run the subtask agent and persist traces. Returns (result, elapsed, cost)."""
        ...
        self._persist_trace(
            f"task_agent-{parent_id}-sub{child_id}",
            prompt,
            result,
        )
    ...
    async def _retry_subtask(
        ...
        self._persist_trace(
            f"task_agent-{parent_id}-sub{child_id}-retry",
            retry_prompt,
            result,
        )
    ...
    def _escalate(self, verdict: ValidationVerdict) -> None:
        ...
        self._update_task(
            ...
            status=TaskStatus.IN_PROGRESS,
            comment=(
                f"**Task agent escalation (supervisor) — needs human review**\n\n"
    ...
    def _persist_trace(event_id: str, prompt: str, result) -> None:
        ...
        if result and result.trace_events:
            _write_trace("task_agent", event_id, result.trace_events)
