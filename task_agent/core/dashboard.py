...
def _extract_numeric_id(event_id: str) -> tuple[str, str]:
    ...
    Examples:
        "task_agent-123-20260215"  -> ("task_agent", "123")
    """
    if "task_agent" in event_id:
        nums = re.findall(r"\b(\d+)\b", event_id)
        return "task_agent", nums[0] if nums else ""
    return "", ""
...
def config_to_snapshot(config: Any) -> dict:
    ...
    model, concurrency, budget, and task_agent flow status.
    """
    if config is None:
        return {}
    try:
        flows = {}
        flow_models = {}
        fc = getattr(config, "task_agent", None)
        if fc is not None:
            flows["task_agent"] = getattr(fc, "enabled", False)
            fm = getattr(fc, "model", "")
            if fm:
                flow_models["task_agent"] = fm
        return {
...
_SESSIONS_FILE = DATA_DIR / "state" / "task_agent_sessions.json"
_LOG_DIR = DATA_DIR / "logs"


def _read_sessions() -> dict:
    """Read task-agent sessions from disk (called in a thread)."""
...
def _find_subtask_trace(parent_id: str, subtask_id: str) -> Path | None:
    ...
    p = TRACES_DIR / "task_agent" / f"task_agent-{parent_id}-sub{subtask_id}.jsonl"
...
def mount_dashboard(  # pylint: disable=too-many-locals,too-many-statements
    ...
    *live_state_file*, when set, makes ``/api/live`` read from a JSON file
    ...
    async def api_sessions() -> JSONResponse:
        """Return task_agent session state from the sessions file."""
        try:
    ...
    async def api_subtask_trace(parent_id: str, subtask_id: str) -> JSONResponse:
        ...
        if trace_path:
            ...
            return JSONResponse(content={
                "parent_id": parent_id, "subtask_id": subtask_id,
        ...
        session = sessions_data.get("sessions", {}).get(
            str(parent_id), {}
        )
        sid = int(subtask_id)
        events = [
            e for e in session.get("event_log", [])
            if e.get("subtask_id") == sid
        ]
        if not events:
        ...
        return JSONResponse(content={
            "parent_id": parent_id, "subtask_id": subtask_id,
            "events": events, "stats": {}, "source": "event_log",
        })
