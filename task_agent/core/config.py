...
@dataclass
class TaskAgentFlowConfig(FlowConfig):
    """Task-agent orchestration settings."""
...
class Config:
    ...
    task_agent: TaskAgentFlowConfig = field(default_factory=TaskAgentFlowConfig)
...
def _parse_task_agent_config(data: dict[str, Any]) -> TaskAgentFlowConfig:
    common = _parse_flow_common(data, {"poll_interval": 120})
    return TaskAgentFlowConfig(
        **common,
...
def load_config(config_path: str | Path | None = None) -> Config:
    ...
    return Config(
        task_agent=_parse_task_agent_config(flows_data.get("task_agent", {})),
...
def validate_config(config: Config) -> list[str]:
    ...
    # Validate task_agent if enabled
    if config.task_agent.enabled:
        errors.extend(validate_flow_config("task_agent", config.task_agent))
