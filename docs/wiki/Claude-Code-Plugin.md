...
**Verification flow:** Each command in `golem.md`'s `verify` section is run with its specified timeout. If a command fails, the AI revises the entry and retries up to twice. Commands that cannot be made to pass are removed and the user is warned.
...
### verify
- **role:** `test` | **cmd:** `["pytest", "tests/", "-x", "-q"]` | **timeout:** 120
...
- If the daemon is running and repo is attached: prints `[Golem] Daemon active. Repo attached. Use /golem:run to delegate tasks or /golem:status to check running tasks.`
